...
import alchyr.taikoedit.audio.mp3.decoders.LayerIDecoder;
import alchyr.taikoedit.audio.mp3.decoders.LayerIIDecoder;
import alchyr.taikoedit.audio.mp3.decoders.LayerIIIDecoder;
...
public class PreloadMp3Bitstream {
    ...
    public static float progress = 0; //load progress
    ...
    private void preload()
    {
        ...
        try
        {
            ...
            while (h != null)
            {
                ...
                totalBytes += length;
                progress = totalBytes / (approximateBytes * 7.5f);
    ...
    public float seekTime(float time)
    {
        ...
        if (index >= 0)
        {
            currentFrame = index; //Precise match (Rare)
        }
        else
        {
            currentFrame = -index - 1; //No precise match (expected result)
        }

        if (currentFrame < frameTimes.size()) //move back to correct frame
        {
    ...
    /**
     * Get next 32 bits from bitstream. They are stored in the headerstring. syncmode allows Synchro flag ID The returned value is
    ...
    int syncHeader(byte syncmode) throws BitstreamException {
        ...
        //headerstring = aaaaaaaabbbbbbbbcccccccc where a = syncbuf[0], b = syncbuf[1], c = syncbuf[2]
        headerstring = syncbuf[0] << 16 & 0x00FF0000 | syncbuf[1] << 8 & 0x0000FF00 | syncbuf[2] & 0x000000FF;

        do {
            //shift left 8 additional bits to aaaaaaaabbbbbbbbcccccccc________
            headerstring <<= 8;

            //read one more byte
            if (readBytes(syncbuf, 3, 1) != 1) throw newBitstreamException(STREAM_EOF, null);

            //append one more byte (aaaaaaaabbbbbbbbccccccccdddddddd)
            headerstring |= syncbuf[3] & 0x000000FF;

            //Test if valid sync
            sync = isSyncMark(headerstring, syncmode, syncword);
        } while (!sync); //Repeat process until valid sync, shifting 8 bits each time
